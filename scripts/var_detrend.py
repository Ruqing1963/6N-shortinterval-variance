...
import csv
with open(os.path.join(_DATA,"fano_decomposition.csv"),"w",newline="") as fh:
    w=csv.writer(fh); w.writerow(["H","F_total","F_local_detrended","F_trend"])
    for row in rows: w.writerow([row["H"],"%.6f"%row["F_total"],"%.6f"%row["F_local"],"%.6f"%row["F_trend"]])
print("wrote %s/fano_decomposition.csv  (smoothing W=%d)"%(_DATA,W))
