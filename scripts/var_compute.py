...
json.dump(dict(NMAX=NMAX,LIMIT=LIMIT,ntwin=ntwin,p0=p0,G=G,Kmax=Kmax,
    fano=fano, idcheck=idcheck,
    Ck=Ck.tolist(), C_HL=C_HL.tolist(),
    sumCm=float((Ck-1).sum()), sumCHL=float((C_HL-1).sum()),
    meanCm=float(Ck.mean()), meanCHL=float(C_HL.mean()),
    median_resid=float(np.median(np.abs(resid)))),
    open(os.path.join(_DATA,"var_results.json"),"w"))
np.savez(os.path.join(_DATA,"var_arrays.npz"),kk=kk,Ck=Ck,C_HL=C_HL,
         Hs=np.array([f["H"] for f in fano]),F=np.array([f["F"] for f in fano]),
         mu=np.array([f["mean"] for f in fano]),var=np.array([f["var"] for f in fano]))
