...
def read_csv(name):
    ...
    return cols
summ={r["parameter"]:r["value"] for r in csv.DictReader(open(os.path.join(_DATA,"summary.csv")))}
p0=float(summ["p0_global_twin_rate"]); G=float(summ["G_generic_4tuple_correlation"])

pc=read_csv("pair_correlation.csv")
kk=pc["k"]; Ck=pc["C_measured"]; C_HL=pc["C_HL_prediction"]
fd=read_csv("fano_decomposition.csv")
Hd=fd["H"]; Ftot=fd["F_total"]; Floc=fd["F_local_detrended"]; Ftr=fd["F_trend"]

# HL-predicted Fano via exact variance identity, from C_HL
def F_pred(H,Cvec):
    H=int(H); k=np.arange(1,H)
