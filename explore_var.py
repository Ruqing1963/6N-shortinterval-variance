...
np.savez(os.path.join(_DATA,"explore_var.npz"),Hs=np.array(Hs),fano=np.array([f[4] for f in fano]),
         mu=np.array([f[2] for f in fano]),A=A,p0=p0,NMAX=NMAX,ntwin=ntwin,Kmax=Kmax)
